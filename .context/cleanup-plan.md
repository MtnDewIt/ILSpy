...
## Issue 1 — Attribute decode failure

**Root:** `TypeSystem/TypeProvider.cs` → `GetUnderlyingEnumType()` throws `EnumUnderlyingTypeResolveException` when `def` is null (i.e., the enum type is unresolved/external). This propagates up through `CustomAttribute.cs` → `DecodeValue()`, which catches it and sets `hasDecodeErrors = true`, causing `TypeSystemAstBuilder.cs` → `ConvertAttribute()` to replace the entire argument list with the error comment.

**Fix — two-part:**

**Part A (`TypeProvider.cs` ~line 145):** Instead of throwing when `def` is null, default to `PrimitiveTypeCode.Int32`, since `int` is the C# default underlying type for all enums:
```cs
public SRM.PrimitiveTypeCode GetUnderlyingEnumType(IType type)
{
    var def = type.GetEnumUnderlyingType().GetDefinition();
    if (def == null)
        return SRM.PrimitiveTypeCode.Int32; // default enum underlying type
    return def.KnownTypeCode.ToPrimitiveTypeCode();
}
```

**Part B (`CustomAttribute.cs` `DecodeValue()`):** The current catch bails out and returns empty arguments. If you want maximum recovery (decode what *can* be decoded), the exception no longer fires after Part A — so Part B becomes a safety net only. No code change needed there beyond verifying the existing catch remains for `BadImageFormatException`.

---

## Issue 2 — Incorrect class reference when class name collides with declaring type
...
## Issue 3 — Enum output formatting (four sub-issues)

### 3a — `[Flags]` power-of-two values should use `1 << N` notation
...
### 3b — Combined `[Flags]` values should use OR-chains
...
### 3c — Enum-typed switch discriminants cast as `(int)val - offset`
...
### 3d — Inline `(EnumType)intLiteral` expressions
...
### 4c — `((object)value).ToString()` boxing before method call
...
## Issue 5 — XAML/BAML decompilation

**Root:** `ICSharpCode.BamlDecompiler/Rewrite/MarkupExtensionRewritePass.cs` and `XClassRewritePass.cs`.

### 5a — `<Type.Property><Ctor>value</Ctor>...</Type.Property>` children not inlined as attributes
...
### 5b — Root element not replaced with base class + `x:Class`
...
## Issue 6 — XAML partial class / duplicate member output
...
Issues 1, 4a, 4b, and 4c are the smallest/lowest-risk changes. Issues 3 (enum formatting) and 5 (XAML rewrite) are the most complex and should be developed and tested independently. Issue 6 (partial class) depends on Issue 5 being solved first since it relies on `XClassNames` and `GeneratedMembers` being correctly populated.